...
def process(epsilon, k, rand_seed, file_path, ground_truth, a_value):
    ...
    users_indicies = np.random.choice(len(data), int(len(data)*a_value), replace=False)
    ...
    file_result = open(file_path+'/initial_result/'+str(epsilon)+'_'+str(k)+'_'+str(a_value)+'_'+str(rand_seed)+'_result.txt', 'w')
...
if __name__=='__main__':
   # file_name = '../JD_dataset/JD_data_age_sampled.npy'
    ...
    file_path = '../result_'+task+'PTS_all_vary_a/'
    ...
    for iter_ in range(iter_nums):
        result = process(epsilon, top_k_value, iter_, file_path, ground_truth, a_value)
        # exit()
        if result[0] == 0:
            continue
