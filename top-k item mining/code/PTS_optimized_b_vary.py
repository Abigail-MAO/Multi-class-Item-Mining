...
def process(epsilon, k, rand_seed, file_path, ground_truth, b_value):
    ...
    users_indicies = np.random.choice(len(data), int(len(data)*0.2), replace=False)
    ...
    for i in category:
        ...
        if user_amount_per_category>user_amount_estimation[i]*b_value:
    ...
    file_result = open(file_path+'/initial_result/'+str(epsilon)+'_'+str(k)+'_'+str(b_value)+'_'+str(rand_seed)+'_result.txt', 'w')
...
if __name__=='__main__':
    ...
    file_path = '../result_'+task+'PTS_all_vary_b/'
    ...
    for iter_ in range(iter_nums):
        result = process(epsilon, top_k_value, iter_, file_path, ground_truth, b_value)
        # exit()
        if result[0] == 0:
            continue
    ...
    file = open(file_path+'/'+str(epsilon)+'_'+str(top_k_value)+'_'+str(b_value)+'_result.txt', 'w')
