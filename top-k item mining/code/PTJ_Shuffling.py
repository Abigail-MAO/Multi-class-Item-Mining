...
def find_top_k_per_category(epsilon, data, iter_num, k, candidates):
    bucket_length = 4*k
    remain_length = 2*k
    ...
    for i in range(iter_num):
        ...
        user_stats = np.zeros(bucket_length+1, dtype=np.uint32)
       
        for elem_index in range(len(users_data)):
            if users_data[elem_index] in index_record.keys():
                user_stats[index_record[users_data[elem_index]]] += 1
            else:
                user_stats[bucket_length] += 1
        bucket_counts = OUE_initial_perturb(epsilon, user_stats, len(users_data))
        gc.collect()
...
def multi_category_finding(file_name, epsilon, k, seed, ground_truth, file_path):
    ...
    candidates = find_top_k_per_category(epsilon, data, max_iter_num, find_domain, candidates)
...
if __name__ == '__main__':
    ...
    file_path = '../result_'+task+'PTJ_shuffling/'
    ...
    for i in range(iter_nums):
        result = multi_category_finding(file_name, epsilon, top_k_value, i, ground_truth, file_path)
        if not result[0]:
            continue
        f1 += result[0]
        ncg += result[1]
