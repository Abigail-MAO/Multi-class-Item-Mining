...
if __name__=='__main__':
    epsilon = 1
    data = np.load('./SYN1/test_data.npy')
    seed = int(sys.argv[1])
    ...
    for data_index in range(len(data)):
        real_frequency[data[data_index][1]][data[data_index][0]] += 1
    np.save('./SYN1/OUE/real_frequency.npy', real_frequency)
    for index_ in range(1000):
        random_seed = 2024+seed+index_*10
        real_frequency, frequency = first_partition((epsilon, data, random_seed))
        np.save('./SYN1/OUE/frequency_'+str(seed+index_*10)+'.npy', frequency)
        random_seed = 2024+9+index_*10
        real_frequency, frequency = first_partition((epsilon, data, random_seed))
        np.save('./SYN1/OUE/frequency_'+str(9+index_*10)+'.npy', frequency)
    exit()
